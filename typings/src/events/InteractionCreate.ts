...
export default class AvonInteraction extends AvonEvent {
  ...
  private async checkServerPremium(interaction: any): Promise<boolean> {
    if (
      this.client.utils.checkServerPrem(interaction.guildId) === false &&
      this.client.utils.checkServerPremStatus(interaction.guildId) === false
    ) {
      interaction.update({
        embeds: [
          this.client.utils
  ...
  private async checkVote(interaction: any): Promise<boolean> {
  ...
  private checkPermissions(interaction: any): boolean {
    if (
      !interaction.member.permissions.has("ManageGuild") &&
      !this.client.config.owners.includes(interaction.user.id) &&
      this.client.utils.checkActivator(interaction.guildId) !== interaction.user.id
    ) {
      interaction.reply({
        embeds: [
          this.client.utils
  ...
  private checkVoiceChannel(interaction: any): boolean {
    if (
  ...
  private getDispatcher(interaction: any): any {
  ...
  private checkMessageId(interaction: any): boolean {
  ...
  private async checkDjPremium(interaction: any): Promise<boolean> {
    if (
      this.client.utils.checkServerPrem(interaction.guild.id) === false &&
      this.client.utils.checkServerPremStatus(interaction.guild.id) === false
    ) {
    ...
    this.run = async (interaction: any) => {
      if (interaction.isButton()) {
        ...
        if (interaction.customId === `avon_classic_mode`) {
          if (!await this.checkServerPremium(interaction)) return;
          if (!await this.checkVote(interaction)) return;
          if (!this.checkPermissions(interaction)) return;
