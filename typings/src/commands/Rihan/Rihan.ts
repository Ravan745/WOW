    ...
    this.exec = async (message: any, args: any, prefix: any) => {
      ...
      if (args[0].toLowerCase() === `add`) {
          ...
          return message.reply({
            embeds: [
              this.client.utils
                .successEmbed()
                .setDescription(
                  `${this.client.emoji.tick} Successfully **Added** ${user.username} to my **Rihan's List**`
                ),
            ],
          });
        }
          ...
          return message.reply({
            embeds: [
              this.client.utils
                .successEmbed()
                .setDescription(
                  `${this.client.emoji.tick} Successfully **Removed** ${user.username} from my Rihan's List`
                ),
            ],
          });
        }
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          lol.push(
            `**(${i + 1})** [${us.username}](${this.client.config.server}) [ID: ${
              us.id
            }]`
          );
        }
